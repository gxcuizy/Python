...
def get_province(index_href):
    ...
    for province_tr in province_tr_list:
        if province_tr:
            ...
            province_info = 'INSERT INTO xfc_region_copy (name, level, parent_id, gid) VALUES ("' + str(province_name) + '", "' + str(level) + '", "' + str('0') + '", "' + str(province_code) + '");'
...
def get_city(province_href, province_id):
    ...
    for city_tr in city_tr_list:
        if city_tr:
            ...
            city_info = 'INSERT INTO xfc_region_copy (name, level, parent_id, gid) VALUES ("' + str(city_name) + '", "' + str(level) + '", "' + str(province_id) + '", "' + str(city_code) + '");'
...
def get_area(city_href, city_id):
    ...
    for area_tr in area_tr_list:
        ...
        if area_a_info:
            ...
            area_info = 'INSERT INTO xfc_region_copy (name, level, parent_id, gid) VALUES ("' + str(area_name) + '", "' + str(level) + '", "' + str(city_id) + '", "' + str(area_code) + '");'
...
def get_town(origin_url, now_url, area_id):
        ...
        for town_info in town_list:
            ...
            town_sql = 'INSERT INTO xfc_region_copy (name, level, parent_id, gid) VALUES ("' + str(town_name) + '", "' + str(level) + '", "' + str(area_id) + '", "' + str(town_code) + '");\n'
            add_data(town_sql)
        print('乡镇级解析结束！')
